...
1. Add Joi/Zod validation on the Gemini output before saving to MongoDB.
2. Cache match results and invalidate via MongoDB Change Streams when a new document is added.
3. Add a comprehensive Jest + Supertest test suite covering all match rule permutations.
4. Surface item-level match details in the response (which specific items matched vs mismatched).
5. Add a cleanup job to delete orphaned `uploads/` temp files after parsing.
